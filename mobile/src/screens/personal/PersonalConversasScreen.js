...
const PersonalConversasScreen = () => {
  ...
  const loadConversas = async () => {
    try {
      const response = await conversaAPI.getConversasByProfessor(user.id);
      // Carregar informações dos alunos associados às conversas
      const conversasComAluno = await Promise.all(
      ...
      setConversas(conversasComAluno);
    } catch (error) {
    ...
    <TouchableOpacity style={styles.conversaCard}>
      <View style={styles.conversaHeader}>
        ...
        <View style={styles.conversaInfo}>
          <Text style={styles.conversaTitle}>{item.aluno?.name || 'Aluno'}</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {conversas.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>Você ainda não tem conversas com alunos</Text>
        </View>
      ) : (
        <FlatList
          data={conversas}
...
export default PersonalConversasScreen;